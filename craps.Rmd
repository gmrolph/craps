...
x <- tibble(john_mark = map_int(1:n, ~ roll_two_dice()),
            ishan = map_int(1:n, ~ roll_two_dice())) %>% 
        mutate(ishan_craps = ifelse(ishan %in% c(7,11),1,0),
              john_mark_craps = ifelse(john_mark %in% c(7,11),1,0),
              winner = (ifelse(john_mark_craps & !ishan_craps, "John Mark",
                     ifelse(!john_mark_craps & ishan_craps, "Ishan", "Tie")))) %>% 
  mutate(contest = rep(1:number_of_contests, each = contest_size)) %>% 
  group_by(contest) %>% 
  summarize(john_mark_craps = sum(winner == "John Mark")) %>% 
  count(john_mark_craps >= 4)
